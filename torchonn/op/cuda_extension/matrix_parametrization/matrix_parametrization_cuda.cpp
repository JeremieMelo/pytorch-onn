...
template <typename T> inline T calPhi(const T u1, const T u2) {
  T u1_abs = abs(u1);
  T u2_abs = abs(u2);
  T phi = 0;
...
void decompose_francis_forward(at::Tensor U, at::Tensor delta_list,
                               at::Tensor phi_mat) {
  ...
  TORCH_CHECK(delta_list.size(-1) == U.size(-1),
              "delta_list should have length N")
  TORCH_CHECK(phi_mat.size(-2) == phi_mat.size(-1) &&
                  phi_mat.size(-1) == U.size(-1),
              "phi mat should have size NxN")
  ...
  AT_DISPATCH_FLOATING_TYPES(
      U.scalar_type(), "unitaryDecomposeFrancisCUDALauncher", ([&] {
        unitaryDecomposeFrancisCUDALauncher<scalar_t>(
            BS, N, U.data_ptr<scalar_t>(), delta_list.data_ptr<scalar_t>(),
...
torch::Tensor reconstruct_francis_forward(
    //   torch::Tensor U,
    torch::Tensor delta_list, torch::Tensor phi_mat) {
  // TORCH_CHECK(U.type().is_cuda(), "U must be a CUDA tensor")
  // TORCH_CHECK(U.size(-2) == U.size(-1), "U must be a square tensor")
  // TORCH_CHECK(U.is_contiguous(), "U must be contiguous")
  TORCH_CHECK(phi_mat.is_contiguous(), "phi mat must be contiguous")
  // TORCH_CHECK(delta_list.size(-1) == U.size(-1), "delta_list should have
  // length N") TORCH_CHECK(phi_mat.size(-2) == phi_mat.size(-1) &&
  // phi_mat.size(-1) == U.size(-1), "phi mat should have size NxN")
  ...
  AT_DISPATCH_FLOATING_TYPES(
      W.scalar_type(), "unitaryReconstructFrancisCUDALauncher", ([&] {
        unitaryReconstructFrancisCUDALauncher<scalar_t>(
            BS, N, W.data_ptr<scalar_t>(), phi_mat_cos.data_ptr<scalar_t>(),
...
void decompose_clements_forward(at::Tensor U, at::Tensor delta_list,
                                at::Tensor phi_mat) {
  // TORCH_CHECK(U.type().is_cuda(), "U must be a CUDA tensor")
  TORCH_CHECK(U.size(-2) == U.size(-1), "U must be a square tensor")
  TORCH_CHECK(U.is_contiguous(), "U must be contiguous()")
  ...
  int BS = U.numel() / (N * N);
  AT_DISPATCH_FLOATING_TYPES(
      U.scalar_type(), "unitaryDecomposeClementsCUDALauncher", ([&] {
...
torch::Tensor reconstruct_clements_forward(
  ...
  int N = delta_list.size(-1);
  auto phi_mat_cos =
      torch::cos(phi_mat); // clement-style interleaved checkerboard
  auto phi_mat_sin = torch::sin(phi_mat);
  auto W = torch::ones_like(delta_list).diag_embed().contiguous();
  int BS = delta_list.numel() / N;
  phi_mat = phi_mat.contiguous();
  ...
  AT_DISPATCH_FLOATING_TYPES(
      W.scalar_type(), "unitaryReconstructClementsCUDALauncher", ([&] {
...
void decompose_reck_forward(at::Tensor U, at::Tensor delta_list,
                            at::Tensor phi_mat) {
  // TORCH_CHECK(U.type().is_cuda(), "U must be a CUDA tensor")
  TORCH_CHECK(U.size(-2) == U.size(-1), "U must be a square tensor")
  TORCH_CHECK(U.is_contiguous(), "U must be contiguous()")
  ...
  int BS = U.numel() / (N * N);
  AT_DISPATCH_FLOATING_TYPES(
      U.scalar_type(), "unitaryDecomposeReckCUDALauncher", ([&] {
...
torch::Tensor reconstruct_reck_forward(
  ...
  phi_mat = phi_mat.contiguous();
  AT_DISPATCH_FLOATING_TYPES(
      W.scalar_type(), "unitaryReconstructReckCUDALauncher", ([&] {
...
//     delta_list = delta_list.contiguous();
//     int BS = U.numel() / (N * N);
//     AT_DISPATCH_FLOATING_TYPES(U.scalar_type(),
//     "unitaryDecomposeReckCUDALauncher", ([&] {
//         unitaryDecomposeReckCUDALauncher<scalar_t>(
//             BS,
//             N,
//             U.data_ptr<scalar_t>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("decompose_francis", &decompose_francis_forward,
        "matrix parametrization in Francis style");
  m.def("reconstruct_francis", &reconstruct_francis_forward,
        "matrix parametrization reconstruct in Francis style");
  m.def("decompose_clements", &decompose_clements_forward,
        "matrix parametrization in Clements style");
  m.def("reconstruct_clements", &reconstruct_clements_forward,
        "matrix parametrization reconstruct in Clements style");
  m.def("decompose_reck", &decompose_reck_forward,
        "matrix parametrization in Reck style");
  m.def("reconstruct_reck", &reconstruct_reck_forward,
        "matrix parametrization reconstruct in Reck style");
}
